...
function Layout() {
  ...
  const saveNote = async(note, index) => {
    ...
    await fetch(
      "https://2h7tqzeqgcgrhbp426w2b5fqai0jnbjy.lambda-url.ca-central-1.on.aws/",
  ...
  const deleteNote = async (index, user) => {
    ...
    await fetch(
      "https://4d7urfxvqdu5skqhma2l4fga4u0uojvw.lambda-url.ca-central-1.on.aws/",
  ...
  const getNotes = async (profile, aToken) => {
    ...
    const res = await fetch(
      `https://5wwcbm6everz3jghab4x432uvy0wgjqw.lambda-url.ca-central-1.on.aws?email=${email}`,
      {
        // mode:'no-cors',
        method: "GET",
        headers: {
